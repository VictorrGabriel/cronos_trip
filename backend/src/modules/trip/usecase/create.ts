...
import { getEntityFromCreateDTO, getTripResponseDTO } from "./mappers";
...
export const usecaseCreate: UsecaseCreate = async (
  ...
  const tripEntity = getEntityFromCreateDTO(data, publicId, user);
  const trip = await tripRepository.create(tripEntity);

  const responseTrip: TripResponseDTO = getTripResponseDTO(trip, data.userId);
